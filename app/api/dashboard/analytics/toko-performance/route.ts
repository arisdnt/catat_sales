...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[Dashboard Analytics] Error calling get_dashboard_toko_performance:', error)
      
      if (error.message?.includes('function get_dashboard_toko_performance') || error.code === '42883') {
    ...
    console.error('[Dashboard Analytics] Unexpected error in toko-performance API:', error)
    return NextResponse.json(
      { 
        success: false, 
