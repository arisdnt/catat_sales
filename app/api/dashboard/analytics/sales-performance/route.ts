...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[Dashboard Analytics] Error calling get_dashboard_sales_performance:', error)
      
      // If function doesn't exist, return empty data
      if (error.message?.includes('function get_dashboard_sales_performance') || error.code === '42883') {
    ...
    console.error('[Dashboard Analytics] Unexpected error in sales-performance API:', error)
    return NextResponse.json(
      { 
        success: false, 
